...
export function Visits() {
  ...
  return (
    <div>
      <Topbar title="Visits" subtitle="Track scheduled, active, completed, and delayed visits" />
      ...
      <div className="grid gap-6 xl:grid-cols-[1.2fr_1fr]">
        ...
        <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-soft">
          <h3 className="mb-3 text-lg font-semibold text-slate-900">Visit records</h3>
            ...
            {current.map((visit) => (
              <article key={visit.id} className="rounded-xl border border-slate-200 p-3">
                ...
                <p className="mt-1 text-sm text-slate-500">
                  Agent: {agents.find((agent) => agent.id === visit.agentId)?.name}
                </p>
                <p className="text-sm text-slate-500">Client: {visit.clientName}</p>
